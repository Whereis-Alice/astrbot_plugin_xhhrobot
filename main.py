...
from .insight_card import (
    InsightCardRenderer,
    InsightCardResult,
    available_insight_card_resolutions,
...
class XhhRobotPlugin(Star):
    ...
    def _comment_insight_snapshot(self) -> dict[str, Any]:
        ...
        default_card_resolution = normalize_insight_card_resolution(
            "",
            self._str_cfg("analytics.insight_card_resolution", "high"),
        )
        payload["card"] = {
            ...
            "default_resolution": default_card_resolution,
            "resolutions": available_insight_card_resolutions(),
        }
        return payload
    ...
    async def _wait_for_comment_insight_completion(
        self,
        job_id: str,
        *,
        timeout_sec: int | None = None,
    ) -> dict[str, Any]:
        """Wait for an LLM-tool request without cancelling its background job."""
        requested_job_id = str(job_id or "").strip()
        snapshot = self._comment_insight_snapshot()
        if requested_job_id and str(snapshot.get("job_id") or "") != requested_job_id:
            return snapshot
        if str(snapshot.get("state") or "") != "running":
            return snapshot
        ...
        timeout = (
            self._int_cfg("analytics.insight_tool_wait_sec", 90, 0, 600)
            if timeout_sec is None
            else max(0, min(600, int(timeout_sec)))
        )
        timed_out = False
        if timeout > 0:
            try:
                ...
                # Keep the shared analysis running for WebUI and later status calls.
                timed_out = True

        snapshot = self._comment_insight_snapshot()
        if requested_job_id and str(snapshot.get("job_id") or "") != requested_job_id:
            return snapshot
        snapshot["conversation_wait"] = {
            "attempted": timeout > 0,
            "timeout_sec": timeout,
            "timed_out": timed_out,
            "completed": str(snapshot.get("state") or "") == "complete",
        }
        return snapshot
    ...
    async def _render_comment_insight_card(
        ...
        *,
        ...
        return await renderer.render(
            ...
            default_resolution=self._str_cfg(
                "analytics.insight_card_resolution", "high"
            ),
        ...
        async with lock:
            ...
            try:
                ...
                if not profile.get("avatar"):
                    login_avatar = self._account_avatar_url(
                        auth.avatar,
                        XhhClient.parse_cookie_header(auth.cookie).get("avatar")
                    )
                    if login_avatar:
                        profile["avatar"] = login_avatar
                ...
                if has_profile_details:
                    ...
                    updated_auth = replace(
                        auth,
                        nickname=str(profile.get("nickname") or auth.nickname).strip(),
                        avatar=str(profile.get("avatar") or auth.avatar).strip(),
                    )
                    if updated_auth != auth:
                        self.auth = updated_auth
    ...
    @staticmethod
    def _account_avatar_urls(*values: Any) -> list[str]:
        preferred_keys = (
            "original",
            "original_url",
            "origin",
        ...
        urls: list[str] = []
        for value in values:
            urls.extend(candidates(value))
        return unique_strings(urls)

    @classmethod
    def _account_avatar_url(cls, *values: Any) -> str:
        urls = cls._account_avatar_urls(*values)
    ...
    async def _refresh_account_avatar(self, *, force: bool = False) -> str:
        profile = dict(getattr(self, "_account_profile", {}) or {})
        if not str(profile.get("avatar") or "").strip():
            await self._refresh_account_profile(force=force)
            profile = dict(getattr(self, "_account_profile", {}) or {})
        auth = getattr(self, "auth", None)
        sources = self._account_avatar_urls(
            profile.get("avatar"),
            auth.avatar if auth is not None else "",
            (
                XhhClient.parse_cookie_header(auth.cookie).get("avatar")
                if auth is not None
                else ""
        ...
        async with lock:
            ...
            if (
                ...
                return cached_data
            last_error: XhhError | None = None
            for source in sources:
                try:
                    payload = await fetch_image(
                        source,
                        max_bytes=ACCOUNT_AVATAR_MAX_BYTES,
                    )
                    if min(payload.width, payload.height) < ACCOUNT_AVATAR_MIN_EDGE:
                        raise XhhError(
                            "账号头像返回了异常小的占位图。",
                            retryable=False,
                        )
                ...
                self._account_avatar_error = ""
                return data_url
            self._account_avatar_error = str(last_error or "账号头像读取失败。")
            if cached_data and cached_source in sources:
                return cached_data
            if last_error is not None:
                raise last_error
            raise XhhError("账号头像读取失败。", retryable=True)
