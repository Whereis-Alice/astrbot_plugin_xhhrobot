...
def build_exploratory_report(
    *,
    ...
    evidence_topics = themes or top_topics
    evidence = {
        "mode": "exploratory",
        "scope": {
        ...
        "sentiments": [
            {
                "key": name,
                "label": _SENTIMENT_LABELS[name],
                "count": count,
                "percentage": percentage(count),
            }
            for name, count in sentiment_payload.items()
        ],
        "intents": [
            {
                "key": name,
                "label": _INTENT_LABELS[name],
                "count": count,
                "percentage": percentage(count),
            }
            for name, count in sorted(
                intent_payload.items(),
                key=lambda item: (-item[1], _INTENT_LABELS[item[0]]),
            )
            if count > 0
        ],
        "topics": evidence_topics[:8],
        "basis": "情绪、意图和话题均以本轮已分析评论为分母。",
    }
    return {
    ...
    keyword_breakdown = [
        {
            ...
            "count": sum(
                1
                for result in deterministic.values()
                if any(
                    str(hit).casefold() == keyword.casefold()
                    for hit in result.get("keyword_hits") or ()
                )
            ),
        }
    ...
    emoji_breakdown = [
        {
            ...
            "count": sum(
                ...
                for result in deterministic.values()
                if any(
                    canonical_emoji_token(hit).casefold() == token.casefold()
                    for hit in result.get("emoji_hits") or ()
                )
            ),
        }
        for token in criteria.emoji_tokens
    ]

    def total_percentage(count: int) -> float:
        return round((count / total * 100.0) if total else 0.0, 2)

    evidence = {
        "mode": "directed",
        "scope": {"archived": total},
        "layers": [
            {
                "key": "keyword",
                "label": "直接文字命中",
                "count": len(keyword_keys),
                "percentage": total_percentage(len(keyword_keys)),
                "items": keyword_breakdown,
            },
            {
                "key": "emoji",
                "label": "表情明确命中",
                "count": len(emoji_keys),
                "percentage": total_percentage(len(emoji_keys)),
                "items": emoji_breakdown,
            },
            {
                "key": "semantic",
                "label": "语义近义补充",
                "count": len(semantic_matches),
                "percentage": total_percentage(len(semantic_matches)),
                "items": [],
            },
        ],
        "keyword_emoji_overlap": len(keyword_keys & emoji_keys),
        "deterministic_union": len(deterministic_keys),
        "final_union": len(union_keys),
        "final_percentage": total_percentage(len(union_keys)),
