...
小黑盒表情在接口文本中使用 `[cube_表情名]` 或 `[heygirl_表情名]` 标记，例如标准表情是 `[cube_吐]`，不是 `[cube_吐血]`。插件会优先使用账号当前可用的表情列表，并以内置标准目录兜底；模型或工具生成了不支持的标记时，会降级为普通可读文本。可以调用 `xhh_get_emojis` 查询当前账号实际可用的表情名称，不要根据通知消息里显示的文字臆造表情名。
