...
| `ai.allow_external_search` | 控制回复和自动评论是否可以自主调用已安装的联网搜索工具核实信息，默认开启。 |
| `direct_messages.enabled` | 控制私信自动回复，默认关闭。 |
| `auto_browse.enabled` | 控制自主巡帖，默认关闭。 |
| `tools.enable_write_tools` | 控制 LLM 写工具，默认关闭。 |
| `media.preserve_remote_image_bytes` | 控制 HTTP 图片是否优先按原始字节上传，默认开启。 |
...
WebUI 的“消息数据库”页面支持按数据集、关键词、方向、来源、状态、用户 ID 和帖子 ID 分页筛选，页脚会显示记录范围、当前页和总页数。关闭 `webui.show_message_content` 后，页面只显示隐藏占位、字符数、ID、时间和状态。
