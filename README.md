...
| 评论洞察 | 无需预设关键词即可自动发现主要话题、情绪、问题、建议、夸奖、吐槽和争议，也可按指定主题做精确统计。 |
| WebUI | 提供终端风运行面板、扫码登录、队列控制、消息统计、筛选查询和评论洞察。 |
...
- “评论洞察”留空主题即可自动归纳主要话题、情绪、问题、建议、夸奖、吐槽和争议；填写主题后改为定向统计。两种模式都可按帖子、来源和时间筛选，并查看后台进度、取消任务和代表样例。
...
| `analytics` | SQLite 保留时间、容量、查询上限，以及评论语义洞察模型、批次和样例数量。 |
