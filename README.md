...
| 评论洞察 | 对归档评论做关键词、小黑盒表情和可选 LLM 语义分类，给出去重并集、占比与代表样例。 |
...
| `xhh_comment_insights` | 按自然语言主题分析归档评论，例如夸奖、吐槽、态度或特定话题；统计关键词、标准表情、语义近义表达、去重占比和代表样例，并支持查询进度与取消后台任务。 |
...
- “评论洞察”按主题、关键词、标准表情、帖子、来源和时间范围统计归档评论，可查看后台进度、取消任务和代表样例。

关闭 `webui.show_message_content` 后，消息数据库只显示隐藏占位、字符数、ID、时间和状态，评论洞察也不会向页面返回代表评论原文。

### 评论洞察

评论洞察只分析插件 SQLite 中已经归档的外部用户评论，不会自动抓取启用插件前的小黑盒全部历史。统计分为三层：
...
语义分析在后台分批运行，关闭页面不会取消任务；已完成结果会写入 `semantic_comment_cache`，相同主题、关键词、表情、模型和评论正文再次分析时直接复用。运行中可以在 WebUI 点击“取消分析”，也可以让管理员直接用自然语言要求机器人分析评论，模型会选择 `xhh_comment_insights`；该工具仍受当前账号私密读取保护，仅管理员可调用。工具的 `status` 模式查看进度与结果，`cancel` 模式取消任务。打开洞察页面本身不会调用模型；取消勾选“使用模型补充语义表达”时只做本地统计。

| 配置页中文名 | 配置键 | 说明 |
| --- | --- | --- |
| 启用评论语义洞察 | `analytics.semantic_insights_enabled` | 控制是否允许主动运行 LLM 语义分类；关闭后本地文字和表情统计仍可使用。 |
| 评论洞察模型 | `analytics.semantic_provider_id` | 选择语义二分类模型；留空时沿用“回复模型”。 |
| 语义分析单批评论数 | `analytics.semantic_batch_size` | 每次模型请求包含的评论数，默认 20。 |
| 单次语义分析评论上限 | `analytics.semantic_max_comments_per_run` | 限制单次交给模型的候选评论；`0` 表示不限，确定性统计始终覆盖全部筛选结果。 |
| 洞察代表样例上限 | `analytics.insight_example_limit` | 控制报告最多显示多少条代表评论。 |

语义覆盖不足 100% 时，报告中的并集和占比是“已确认下限”，不代表未纳入本轮的评论都不匹配。模型分类结果用于统计参考，不会自动修改、回复或删除任何小黑盒内容。
