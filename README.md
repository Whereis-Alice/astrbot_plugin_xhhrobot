...
- “运行状态”使用紧凑概览带展示账号头像与公开资料、待处理、评论归档和私信归档，下方继续提供账号、队列和自动功能明细，并可停止或继续后台任务、取消全部或指定帖子的待处理评论。点击右上角“刷新”会重新读取当前账号昵称、头像、等级、关注数、粉丝数、帖子数等实际可用的公开资料，改名后无需重新登录；资料接口暂时失败时保留上次结果，不影响后台任务。头像由插件服务器受限读取后返回给 WebUI，不会让浏览器直接访问账号头像地址；异常小图不会被缓存，读取失败时显示昵称首字占位。
