...
这些图片会作为标准 AstrBot `Image` 消息段传入当前人设和视觉模型；支持的视觉模型会直接接收 PNG、JPG、WebP 等图片。插件不会把消息中心顶层的 `img`、`imgs` 或 `images` 字段直接当成评论图片，因为这些字段通常属于关联帖子。评论回复会按评论 ID读取帖子评论树，楼中楼会按根评论 ID补查，再分别传入当前评论、被回复评论和原帖图片。如果帖子或评论包含 GIF，插件会转换为首帧 PNG 后再交给模型。GIF 下载或转换失败不会阻断文字回复，只会在 AstrBot 后台日志记录并跳过该图片。小黑盒返回的单字段图片和拼接 URL 也会自动拆分。
