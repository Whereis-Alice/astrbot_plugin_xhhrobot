...
| `xhh_comment_insights` | 留空主题时自动探索归档评论中的主要话题、情绪、问题、建议和争议；填写主题时统计夸奖、吐槽、态度或特定话题，并支持查询进度与取消后台任务。 |
| `xhh_render_comment_insight_card` | 将最近一次已完成的评论洞察渲染为图片卡片并发送到当前会话；支持小黑盒终端、赛博朋克、编辑部报告和数据指挥台主题。 |
...
卡片会输出包含总览、情绪与意图、主要话题、问题、建议、争议、发现和代表评论的完整长图，并限制各区块的展示数量，避免超长数据造成图片失控。关闭“在 WebUI 显示消息正文”后，从 WebUI 生成的卡片不会包含评论原文；LLM 工具仍按当前账号私密读取权限保护，仅管理员默认可用。
...
| `analytics` | SQLite 保留时间、容量、查询上限，以及评论语义洞察模型、批次、样例和图片卡片主题。 |
