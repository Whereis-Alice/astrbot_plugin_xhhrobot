...
| 图片与富文本 | 将评论、被回复评论、原帖和私信图片交给视觉模型；支持网络图片、本地图片、Base64 图片、有序富文本发帖与网络图原字节上传。 |
| 消息数据库 | 使用 SQLite 归档评论和私信，保存去重结果、处理状态和机器人发出的评论，并通过 WebUI 或 LLM 工具查询。 |
| 评论洞察 | 无需预设关键词即可自动发现主要话题、情绪、问题、建议、夸奖、吐槽和争议，也可按指定主题做精确统计，并把结果渲染为主题图片卡片。 |
| WebUI | 提供终端风运行面板、扫码登录、队列控制、消息统计、筛选查询、评论洞察和卡片预览。 |
...
| 配置页中文名 | 配置键 | 建议 |
| --- | --- | --- |
| 回复模型 | `ai.provider_id` | 选择负责小黑盒回复的模型；留空时使用当前或默认提供商。 |
| 回复人设 | `ai.persona_id` | 选择已有 AstrBot 人设；留空时使用小黑盒会话或 AstrBot 默认人设。 |
| 启用 AstrBot 标准事件 | `event_bridge.enabled` | 建议保持开启，让评论和私信进入正常消息链。 |
| 允许回复的用户 ID | `filters.allowed_user_ids` | 初次使用时只填写测试用户的小黑盒 ID。 |
| 允许回复所有用户 | `filters.allow_all_users` | 确认回复效果和频率后再考虑开启。 |
| 回复自己帖子下的普通评论 | `filters.reply_to_own_post_comments` | 控制是否回复当前账号自己帖子下没有明确提及机器人的普通评论。 |
| 允许回复时使用外部搜索 | `ai.allow_external_search` | 允许回复和自动评论自主调用已安装的联网搜索工具核实信息。 |
| 启用私信自动回复 | `direct_messages.enabled` | 默认关闭；开启后处理新的好友私信。 |
| 启用自动巡帖 | `auto_browse.enabled` | 默认关闭；建议先使用预览模式。 |
| 启用写操作工具 | `tools.enable_write_tools` | 默认关闭；开启后注册发帖、评论和互动工具。 |
| HTTP 图片优先原字节上传 | `media.preserve_remote_image_bytes` | 默认开启，尽量避免网络缩略图被再次转存。 |

配置页已按账号、人设、标准事件、回复范围、私信、巡帖、工具、图片、统计、WebUI、通知、稳定性和连接分组。完整字段说明以插件配置页为准。

标准事件的“并行处理事件上限”是同时生成和发送的数量保护，“单轮最大回复数”是每轮最多处理的队列数量，两者不是同一个限制。例如单轮最大回复数设为 5、并行处理事件上限保持默认 2 时，插件会先处理 2 条，等其中事件完成后继续处理剩余条目；如果事件超时或插件被停止，未处理条目会继续留在队列中。
...
状态中会显示当前账号、登录来源、公开资料、后台任务、自动回复范围、队列、代理和 LLM 工具状态。每次使用 `/小黑盒状态` 或 `xhh_status` 工具都会立即刷新账号资料，因此改名后不需要重新登录。插件页面还会显示评论与私信统计。
...
标准事件也会经过已启用的 LLM 请求钩子。第三方扩展规则是否生效，取决于相应插件自己的平台、会话和触发条件。
...
每次自动评论、评论回复和私信回复都会向模型提示词加入 AstrBot 服务器当前的日期、时间、星期和时区，用于理解“今天”“最近”“现在”等相对时间。它不会替换帖子或评论自身的发布时间。

配置项 `ai.allow_external_search` 的中文说明是“允许回复时使用外部搜索”。开启后，模型会根据内容自行判断是否需要查资料；插件会从 AstrBot 当前已启用的 LLM 工具中筛选搜索、检索和网页提取工具，因此可以使用已安装的 `astrbot_plugin_anysearch`。搜索只用于获取参考资料，不会把发帖、评论、私信、删除、点赞、关注或收藏等写操作交给这条回复链。搜索不可用或失败时，会回退为普通模型生成，且不会把搜索过程发到小黑盒。

该选项关闭后，回复仍会使用当前时间元数据，但不会调用外部搜索工具。需要注意，联网搜索结果只是外部参考，模型仍应对不确定内容保持谨慎。
...
“单个自己帖子自动回复上限”对应 `filters.max_replies_per_own_post`，默认值为 50。它限制 Bot 对同一个自己帖子下普通评论的自动回复总数，避免热门帖子持续占满队列或触发平台评论频率限制；确认发送和发送结果不确定都会计入，设为 `0` 表示不限。其他帖子中明确 `@` Bot 的评论不受这个单帖上限影响。

插件会在启动时从评论 SQLite 归档恢复已有单帖回复数量，并把后续计数保存在运行状态中。降低上限后，超过新额度的待处理项目会自动标记为跳过；已经开始向小黑盒发送的项目不会被中途移除。
...
这些图片会作为标准 AstrBot `Image` 消息段传入当前人设和视觉模型；支持的视觉模型会直接接收 PNG、JPG、WebP 等图片。插件不会把消息中心顶层的 `img`、`imgs` 或 `images` 字段直接当成评论图片，因为这些字段通常属于关联帖子。评论回复会按评论 ID读取帖子评论树，楼中楼会按根评论 ID补查，再分别传入当前评论、被回复评论和原帖图片。

视觉图片会先由插件服务器下载、校验格式并转为内嵌图片，再交给 AstrBot，避免模型供应商再次下载小黑盒 CDN 地址。异常尾部反斜杠会自动清理；已经失效、返回 404 或无法识别的单张图片只写后台警告并跳过，帖子文字仍会继续参与生成。GIF 会转换为首帧 PNG，小黑盒返回的单字段图片和拼接 URL 也会自动拆分。

### 小黑盒表情

小黑盒表情使用完整的 `[包名_标识符]` 标记，支持 `cube`、`heygirl`、`bigemoji` 和 `grandemoji` 四个包。插件会优先使用当前账号实际返回的表情列表，并以内置的 156 个标准标识符兜底；常见中文、英文和历史名称会在发送前转换为当前可用的标准名称，例如 `[cube_狗头]`、`[cube_doghead]` 会发送为 `[cube_doge]`。未知标记不会被猜测，会降级为普通可读文本。完整标准标识符与兼容规则请查看 [小黑盒表情对照表](./emoji_reference.md)，也可以调用 `xhh_get_emojis` 查询当前账号实际可用的名称。

重复 URL 会自动去重，`ai.max_context_images` 默认最多传入 8 张图片。纯文本模型只能收到图片数量和来源说明，只有支持视觉输入的模型才能识别图片内容。

机器人回复可包含文本和图片。默认情况下，网络图片会先下载原始字节并上传到小黑盒 COS，避免 URL 转存把缩略图作为原图；小黑盒 CDN URL 中的 `imageMogr2`、`imageView2`、水印等处理参数会自动去除。原始下载或上传暂时失败时会回退 URL 转存并写入后台警告。若上游本身只提供缩略图，插件无法还原原图。可通过“HTTP 图片优先原字节上传”（`media.preserve_remote_image_bytes`）关闭此行为。本地图片和 Base64 图片会经过格式、大小与路径检查后上传。
...
错误通知默认关闭。先在配置中填写 `notifications.umo`，再开启 `notifications.notify_on_error`；UMO 必须是 AstrBot 能够主动发送消息的完整会话 ID，例如 `test:FriendMessage:123456`。开启后，后台轮询、自动评论或回复、私信、自动巡帖、标准事件和 LLM 工具调用最终仍失败时，会发送摘要。单张外部图片失效属于可降级输入，只写后台警告，不单独发送错误通知。
...
模型服务的 408、425、429、500、502、503、504、530、Cloudflare Tunnel、上游冷却或临时超时，会在自动巡帖决策阶段进行有限重试。小黑盒消息、推荐流、帖子详情和私信等 GET 读取遇到临时 5xx、非 JSON 500、超时或连接中断也会先短退避重试；最终仍失败时，错误通知会附上已尝试次数。发帖、评论、回复、删除和私信发送等写请求不会因 HTTP 5xx 自动重发，避免重复操作。

| 配置项中文名 | 配置键 | 说明 |
| --- | --- | --- |
| 通知会话 UMO | `notifications.umo` | 接收主动通知的完整 AstrBot 会话 ID；留空时只写后台日志。 |
| 回复成功后通知 | `notifications.notify_on_reply` | 自动回复成功后，向通知会话发送对方内容、当前小黑盒账号回复、图片数量和相关 ID。 |
...
| 启动等待 | 10 分钟 |
| 单轮评论上限 | 1 条 |
| 滚动 24 小时上限 | 3 条（可配置为任意正整数） |
| 同作者冷却 | 72 小时 |
| 同帖子去重 | 30 天 |
| 内容保护 | 关键词、长度、网址、提及、重复评论和提示注入检查 |

自主巡帖是一条独立授权的后台写入路径，不受 `tools.enable_write_tools` 控制，也不会逐条要求聊天确认。发送结果不确定时会停止重试并占用额度，避免重复评论。
...
分析帖子 123456 的评论里有多少人在夸我，并给几条代表评论。
看看最近评论中有多少人在吐槽价格太贵，算出去重后的占比。
把刚才的评论分析做成霓虹街刊卡片。
先生成一篇图文帖草稿供确认，不要发布。
```
...
| `xhh_comment_insights` | 留空主题时自动探索归档评论中的主要话题、情绪、问题、建议和争议；填写主题时统计夸奖、吐槽、态度或特定话题，并支持查询进度与取消后台任务。完成报告会交回当前会话，由 AstrBot 人设和请求钩子组织回答。 |
| `xhh_render_comment_insight_card` | 将最近一次已完成的评论洞察渲染为图片卡片并发送到当前会话；支持小黑盒终端、霓虹街刊、编辑部头版和信号作战室主题，以及标准、高清、超清三档清晰度。 |
...
普通帖子正文、评论回复和私信中的真实换行、字面量 `\\n`、`<br>`、`<br />` 以及段落标签会在发送前统一转换为真正换行，不会作为标记文字显示。需要保留 HTML 格式时，请使用 `content_blocks` 的 `html` 内容块。

针对小黑盒偶尔出现“请添加分区”但帖子实际已经发布的响应，插件会回查当前账号的最近帖子，确认标题和正文后再报告成功。删除已经不存在的帖子会按幂等成功处理，不会重复发送误报。

发帖、评论和私信支持公开 HTTP(S) 图片、Base64 图片以及允许目录中的本地图片。HTTP 图片默认按原始字节上传，下载或上传失败时才回退小黑盒 URL 转存；可用 `media.preserve_remote_image_bytes=false` 恢复旧行为。默认允许插件数据目录、系统临时目录和 AstrBot 的 `data/temp` 目录；本地路径只允许 AstrBot 管理员通过写工具使用。额外目录填写到 `media.allowed_local_roots`，不要将 `/`、`C:\\` 或整个用户目录加入其中。
...
WebUI 使用克制的黑色终端风界面，并在背景绘制低干扰的动态网络。页面保留全部管理能力，在桌面与手机浏览器中均可使用：

- “运行状态”使用紧凑概览带展示账号头像与公开资料、待处理、评论归档和私信归档，下方继续提供账号、队列和自动功能明细，并可停止或继续后台任务、取消全部或指定帖子的待处理评论。点击右上角“刷新”会重新读取当前账号昵称、头像、等级、关注数、粉丝数、帖子数等实际可用的公开资料，改名后无需重新登录；资料接口暂时失败时保留上次结果，不影响后台任务。头像由插件服务器在下载上限内完整读取后返回给 WebUI，不会让浏览器直接访问账号头像地址；异常小图不会被缓存，读取失败时显示昵称首字占位。
- “扫码登录”生成二维码、查看登录状态和清除现有登录凭据。
- “消息数据库”以评论列表为首屏核心，顶部使用一行紧凑统计和评论/私信切换，按关键词、方向、来源、状态、用户 ID 和帖子 ID 分页筛选；页脚显示记录范围、当前页和总页数，详细处理状态位于列表后的折叠区。
- “评论洞察”留空主题即可自动归纳主要话题、情绪、问题、建议、夸奖、吐槽和争议；填写主题后改为定向统计。两种模式都可按帖子、来源和时间筛选，并查看后台进度、取消任务、代表样例，以及选择主题生成图片卡片。

关闭 `webui.show_message_content` 后，消息数据库只显示隐藏占位、字符数、ID、时间和状态，评论洞察也不会向页面返回代表评论原文。

### 评论洞察

评论洞察只分析插件 SQLite 中已经归档的外部用户评论，不会自动抓取启用插件前的小黑盒全部历史。支持两种模式。

#### 自动探索

分析主题、关键词和表情全部留空时进入自动探索。模型会独立判断每条评论的主要话题、正面/中立/负面/混合情绪，以及夸奖、吐槽、提问、建议、玩梗、赞同、反对、经历分享等意图；完成后再汇总主要主题、常见问题、建议、争议、异常信号和代表评论。无需事先知道评论在说什么。

对管理员会话可以直接说：

- `爱丽丝，分析帖子 187917301 的评论，看看大家都在说什么。`
- `分析最近 7 天的评论，告诉我主要话题、情绪、问题和建议。`
- `看看自己帖子下的评论有什么值得注意的。`

爱丽丝应直接调用 `xhh_comment_insights`，将 `topic`、`keywords` 和 `emoji_tokens` 留空，不需要反问关键词。评论超过“单次语义分析评论上限”时，自动探索会跨整个时间范围均匀抽样，避免只分析最新的一段；报告会明确显示入选样本和覆盖率。

#### 定向统计

需要回答“多少人在夸我”“有多少人吐槽价格”或“谁提到了某个角色”时，填写分析主题。定向统计分为三层：

1. **文字命中**：去掉 `[包名_标识符]` 表情标记后，再匹配填写的字面关键词。
2. **表情命中**：按小黑盒标准表情标识符匹配；可以填写中文、英文或历史别名，插件会规范为标准名称。
3. **语义补充**：只把前两层都未命中的评论交给所选模型，判断口语、省略和近义表达是否属于主题。

文字和表情可能同时命中同一条评论，因此报告会单独显示重叠数量。语义补充不重复分析确定性命中项，最终“去重并集”和“样本占比”不会把同一评论重复相加。代表样例按归档中的最近观察时间稳定排序。

模型分析在后台分批运行，关闭页面不会取消任务；已完成的自动分类或定向分类结果会写入 `semantic_comment_cache`，相同模式、模型和评论正文再次分析时直接复用。运行中可以在 WebUI 点击“取消分析”，也可以让管理员直接用自然语言要求机器人分析评论，模型会选择 `xhh_comment_insights`；该工具仍受当前账号私密读取保护，仅管理员可调用。工具的 `status` 模式查看进度与结果，`cancel` 模式取消任务。打开洞察页面本身不会调用模型；定向统计取消勾选“使用模型补充语义表达”时只做本地统计，自动探索必须使用模型。

在群聊或私聊中使用自然语言分析时，工具会按“对话洞察等待时长（秒）”等待本次分析。若分析及时完成，结构化报告会回到触发工具的同一轮 AstrBot Agent，再由该会话当前的人设、上下文和兼容的 LLM 请求钩子生成最终回答。因此，已启用且适用于该会话的世界书等请求钩子可以参与最终表述。后台的逐条分类与统计汇总始终使用中性 JSON 提示词，不读取会话历史，也不套用人设，避免角色口吻影响统计口径。超过等待时长时，分析会继续在后台执行；机器人只会说明仍在分析，可稍后询问进度或在 WebUI 查看结果。

#### 洞察图片卡片

分析完成后，可以在 WebUI 选择主题并点击“渲染卡片”，也可以在管理员会话中直接说：

- `把刚才的评论分析做成霓虹街刊卡片。`
- `把帖子 187917301 的评论分析后生成终端主题图片。`
- `将最近的洞察做成编辑部头版卡片。`

模型会先用 `xhh_comment_insights` 完成分析，再按需调用 `xhh_render_comment_insight_card`。卡片渲染使用 AstrBot 自定义 HTML 渲染器，只对现有结构化报告做排版，不会再次调用 LLM，也不会让模型生成 HTML 或 CSS。

内置主题包括：

| 主题 | 标识 | 风格 |
| --- | --- | --- |
| 小黑盒终端 | `terminal` | 克制的黑绿控制台、运行状态条、扫描线和命令式分析层级。 |
| 霓虹街刊 | `cyberpunk` | 荧光黄、青与洋红构成的不对称街头海报，优先突出大数字和热议主题。 |
| 编辑部头版 | `editorial` | 冷白纸面、双线分隔和衬线标题，按摘要、主要议题和读者声音组织。 |
| 信号作战室 | `command` | 浅色运营简报、深绿状态带和编号区块，优先呈现问题、建议等可执行信号。 |

卡片会输出包含总览、情绪与意图、主要话题、问题、建议、争议、发现、分析依据和代表评论的完整窄幅长图，并限制各区块的展示数量，避免超长数据造成图片失控。卡片采用面向手机阅读的 `680px` 窄幅画布、单列正文和大字号层级；自动探索不再显示固定的大标题，直接从分析摘要进入数据。可以选择标准、高清或超清：默认“高清”兼顾清晰度和文件大小，超清更适合保存或大屏查看。关闭“在 WebUI 显示消息正文”后，从 WebUI 生成的卡片不会包含评论原文；LLM 工具仍按当前账号私密读取权限保护，仅管理员默认可用。

| 配置页中文名 | 配置键 | 说明 |
| --- | --- | --- |
| 启用评论语义洞察 | `analytics.semantic_insights_enabled` | 控制自动探索和定向语义分类；关闭后只保留定向的本地文字与表情统计。 |
| 评论洞察模型 | `analytics.semantic_provider_id` | 选择自动探索与定向分类模型；留空时沿用“回复模型”。 |
| 语义分析单批评论数 | `analytics.semantic_batch_size` | 每次模型请求包含的评论数，默认 20。 |
| 单次语义分析评论上限 | `analytics.semantic_max_comments_per_run` | 自动探索时限制入选样本并跨时间均匀抽样；定向统计时限制语义候选。`0` 表示不限。 |
| 洞察代表样例上限 | `analytics.insight_example_limit` | 控制报告最多显示多少条代表评论。 |
| 对话洞察等待时长（秒） | `analytics.insight_tool_wait_sec` | 管理员在群聊或私聊发起分析后，工具最多等待完成的时间；完成后由当前会话人格和请求钩子总结。`0` 表示立即转为后台分析。 |
| 启用洞察卡片渲染 | `analytics.insight_card_enabled` | 控制 WebUI 和 LLM 工具是否可以把最近一次完成的洞察渲染为图片；关闭后分析本身不受影响。 |
| 洞察卡片默认主题 | `analytics.insight_card_default_theme` | 选择未指定主题时使用的小黑盒终端、霓虹街刊、编辑部头版或信号作战室。 |
| 洞察卡片清晰度 | `analytics.insight_card_resolution` | 选择标准、高清或超清；清晰度越高，图片文件越大。 |
| 洞察卡片代表评论上限 | `analytics.insight_card_example_limit` | 控制图片卡片最多展示多少条代表评论；`0` 表示不展示评论原文。 |

自动探索的情绪、意图和话题占比以本轮已分析样本为分母；定向统计语义覆盖不足 100% 时，并集和占比是“已确认下限”。模型分类结果用于统计参考，不会自动修改、回复或删除任何小黑盒内容。

“运行状态”页面可以直接停止或继续后台任务，也可以取消全部评论待处理队列，或只取消指定帖子 ID 的积压。取消队列只处理“待处理”和“已提交”状态；已经开始发送的项目会保留，消息游标、失败记录、SQLite 归档、统计和登录信息均不会删除。
...
| `/小黑盒启动` | 启动后台轮询。 |
| `/小黑盒停止` | 停止后台轮询。 |
| `/小黑盒清空队列 确认` | 取消全部评论待处理队列；保留正在发送的项目及全部游标、归档和统计。 |
| `/小黑盒清空队列 帖子ID 确认` | 只取消指定帖子下尚未发送的自动回复。 |
| `/小黑盒检查` | 立即执行一轮检查。 |
...
| `filters` | 自动回复范围、自己帖子评论、单帖回复总上限和用户允许或屏蔽列表。 |
...
| `media` | 回复图片数量、HTTP 原字节上传、本地图片大小和允许上传目录。 |
| `analytics` | SQLite 保留时间、容量、查询上限，以及评论语义洞察模型、批次、对话等待时间、样例和图片卡片主题/清晰度。 |
...
- 洞察图片卡片会把经过数量和长度限制的结构化报告提交给 AstrBot 配置的 HTML 渲染服务。卡片不包含 Cookie、代理凭据或数据库路径；不希望报告离开服务器时，请关闭“启用洞察卡片渲染”。
- 停止或卸载插件不会主动删除登录凭据、统计和草稿。
- `/小黑盒退出` 只清除扫码登录凭据，不会删除 SQLite 数据。
- `/小黑盒清空队列` 只放弃尚未发送的评论回复，不会倒退消息游标，也不会删除失败记录或消息数据库。
...
先在配置页设置“单个自己帖子自动回复上限”，避免该帖子继续接收无限量自动回复。需要立即释放现有积压时，可以在 WebUI“运行状态”中取消待处理队列，或执行 `/小黑盒清空队列 帖子ID 确认`；如需先阻止新的轮询，再使用“停止后台任务”或 `/小黑盒停止`。

平台已经提示“您最近的评论频次异常”时，应等待限制自然解除，不要反复手动重试。取消队列不会撤回已经发出的评论，也不会删除正在发送或发送结果不确定的记录。
...
使用联网搜索等 LLM 工具时，AstrBot 可能先产生一个不可见的零宽占位消息。插件会忽略该占位，继续等待工具调用结束后的实际回答；实际回答仍会发送到原来的小黑盒私信会话。请更新到 `v0.6.34` 或更高版本并重载插件。
