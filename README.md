...
小黑盒表情在接口文本中使用 `[cube_表情名]` 或 `[heygirl_表情名]` 标记，例如标准表情是 `[cube_吐]`，不是 `[cube_吐血]`；狗头表情应使用 `[cube_doge]`，插件会兼容模型偶尔生成的 `[cube_狗头]`。插件会优先使用账号当前可用的表情列表，并以内置标准目录兜底；模型或工具生成了不支持的标记时，会降级为普通可读文本。可以调用 `xhh_get_emojis` 查询当前账号实际可用的表情名称，不要根据通知消息里显示的文字臆造表情名。
...
发帖、评论和私信支持公开 HTTP(S) 图片、Base64 图片以及允许目录中的本地图片。默认允许插件数据目录、系统临时目录和 AstrBot 的 `data/temp` 目录；本地路径只允许 AstrBot 管理员通过写工具使用。额外目录填写到 `media.allowed_local_roots`，不要将 `/`、`C:\\` 或整个用户目录加入其中。
