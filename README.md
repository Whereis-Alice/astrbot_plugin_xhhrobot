...
若小黑盒返回“禁止发送消息行为”等明确账号限制，插件会把触发消息记为已跳过，并在本次进程内暂停后续自动私信发送，避免重复触发风控。收信、SQLite 归档和 WebUI 查询仍会继续运行；状态命令与 WebUI 会显示暂停原因。不要通过换 Cookie、代理或重复请求尝试绕过该限制，应在小黑盒 App 或官方渠道处理账号状态，确认恢复后再重载插件。
