...
| `xhh_comment_insights` | 按主题统计关键词、标准表情和语义近义表达，支持启动、查询进度和取消后台任务。 |
...
WebUI 使用黑色终端风界面，并在背景绘制低干扰的动态网络。页面保留全部管理能力，在桌面与手机浏览器中均可使用：

- “运行状态”查看账号、队列、归档和自动功能，并可停止或继续后台任务、取消全部或指定帖子的待处理评论。
- “扫码登录”生成二维码、查看登录状态和清除现有登录凭据。
- “消息数据库”按数据集、关键词、方向、来源、状态、用户 ID 和帖子 ID 分页筛选；页脚显示记录范围、当前页和总页数。
...
语义分析在后台分批运行，关闭页面不会取消任务；已完成结果会写入 `semantic_comment_cache`，相同主题、关键词、表情、模型和评论正文再次分析时直接复用。运行中可以在 WebUI 点击“取消分析”，也可以让管理员调用 `xhh_comment_insights` 的 `cancel` 模式。打开洞察页面本身不会调用模型；取消勾选“使用模型补充语义表达”时只做本地统计。
