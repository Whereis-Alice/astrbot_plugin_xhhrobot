...
| `xhh_render_comment_insight_card` | 将最近一次已完成的评论洞察渲染为图片卡片并发送到当前会话；支持小黑盒终端、赛博朋克、编辑部报告和数据指挥台主题，以及标准、高清、超清三档清晰度。 |
...
| 主题 | 标识 | 风格 |
| --- | --- | --- |
| 小黑盒终端 | `terminal` | 黑色终端、荧光绿、克制的状态网格。 |
| 赛博朋克 | `cyberpunk` | 深色界面、青色与洋红信号、高对比数据标签。 |
| 编辑部报告 | `editorial` | 明亮纸面、编辑部摘要、适合对外分享。 |
| 数据指挥台 | `command` | 石墨色运营面板、青色与琥珀色关键信号。 |

卡片会输出包含总览、情绪与意图、主要话题、问题、建议、争议、发现、分析依据和代表评论的完整长图，并限制各区块的展示数量，避免超长数据造成图片失控。可以选择标准、高清或超清：默认“高清”兼顾清晰度和文件大小，超清更适合保存或大屏查看。关闭“在 WebUI 显示消息正文”后，从 WebUI 生成的卡片不会包含评论原文；LLM 工具仍按当前账号私密读取权限保护，仅管理员默认可用。
...
| 洞察卡片默认主题 | `analytics.insight_card_default_theme` | 选择未指定主题时使用的小黑盒终端、赛博朋克、编辑部报告或数据指挥台。 |
