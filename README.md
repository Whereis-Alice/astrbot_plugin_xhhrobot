...
- **完整图片链**：当前评论、被回复评论、原帖与私信图片可交给视觉模型；回复和写工具支持网络图、本地图片与多图私信。
...
| `ai.max_context_images` | 单次评论回复最多传给模型的图片总数，默认 8；设为 0 可关闭视觉输入。 |
...
插件会从帖子详情再次核验作者 ID。无法确认作者，或作者不是当前登录账号时，普通评论不会发送。`@` 与普通评论使用独立游标，并以“帖子 ID + 评论 ID”二次去重；事件创建和实际外发前还有原子认领，避免同一条评论从两个通知入口、延迟事件或重复 `send()` 触发两次。
...
评论视觉上下文会按“本评论图片、被回复评论图片、帖子图片”的顺序进入消息链，并以同名标签标识来源；重复图片 URL 会自动去重。`ai.max_context_images` 默认限制总数为 8，当前评论图片优先。所选模型或提供商必须支持图片输入，纯文本模型只能收到图片数量和来源提示，无法识别像素内容。
...
若小黑盒返回“禁止发送消息行为”等明确的插件会话请求限制，插件会把触发消息记为已跳过，并在本次进程内暂停后续自动私信发送，避免重复触发风控。这不必然表示手机 App 也无法发送私信。收信、SQLite 归档和 WebUI 查询仍会继续运行；状态命令与 WebUI 会显示暂停原因。不要通过换 Cookie、代理或重复请求尝试绕过该限制，应在小黑盒 App 或官方渠道处理账号状态，确认恢复后再重载插件。
